...
params=Parameters(L=0.56568545, step_size=1.4142135, inverse_mass_matrix=jnp.array([1.0, 1.0]))
...
def run_sampling_algorithm(
    ...
    _, info = jax.lax.scan(lambda s, k: (sampling_algorithm.step(k,s, params=params)), state, keys[1:])
    return info
...
assert(jnp.array_equal(jnp.mean(out.transformed_x, axis=0), [5.004714, 5.018204]))