...
def init_and_kernel(logprob_fn, covariance, mean=None):
    ...
    def step(key: PRNGKey, state: MarginalState, step_size: float):
        ...
        Gamma_1 = Gamma * step_size / (step_size + 2 * Gamma)
        Gamma_3 = (step_size + 2 * Gamma) / (step_size + 4 * Gamma)
        Gamma_2 = Gamma_1 / Gamma_3

        # Propose a new y
        temp = Gamma_1 * (U_x / (0.5 * step_size) + U_grad_x)
        ...
        # Acceptance step
        temp_x = Gamma_1 * (U_x / (0.5 * step_size) + 0.5 * U_grad_x)
        temp_y = Gamma_1 * (U_y / (0.5 * step_size) + 0.5 * U_grad_y)
