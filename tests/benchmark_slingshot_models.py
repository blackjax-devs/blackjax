...
import blackjax
from blackjax.mcmc.slingshot import dual_averaging_step, init_adaptation
...
def make_logit_regression():
    ...
    def logdensity(beta):
        logits_pred = X @ beta
        log_lik = jnp.sum(
            y * jax.nn.log_sigmoid(logits_pred)
            + (1 - y) * jax.nn.log_sigmoid(-logits_pred)
        )
...
def make_hierarchical_model():
    ...
    def logdensity(theta):
        ...
        log_prior_alphas = jnp.sum(
            -0.5 * jnp.log(2 * jnp.pi * tau**2)
            - 0.5 * ((alphas_eval - mu_global) / tau) ** 2
        )

        mu_obs = alphas_eval[group_indices]
        log_lik = jnp.sum(
            -0.5 * jnp.log(2 * jnp.pi * sigma**2) - 0.5 * ((y - mu_obs) / sigma) ** 2
        )

        return (
            log_prior_mu + log_prior_tau + log_prior_sigma + log_prior_alphas + log_lik
        )

    initial_positions = jnp.zeros((16, 11))
    true_params = jnp.concatenate(
        [
            jnp.array([true_mu, jnp.log(true_tau)]),
            alphas,
            jnp.array([jnp.log(true_sigma)]),
        ]
    )
...
def make_neals_funnel():
    def logdensity(theta):
        ...
        var_x = jnp.exp(v)
        log_prior_x = jnp.sum(
            -0.5 * jnp.log(2 * jnp.pi * var_x) - 0.5 * (x**2 / var_x)
        )
...
def run_benchmark(name, logdensity_fn, initial_positions, true_params):
    ...
    def init_chain(pos):
        algo = blackjax.slingshot(
            logdensity_fn, step_size=1.0, num_proposals=num_proposals
        )
    ...
    if "Funnel" in name or "Horseshoe" in name:
        print(
            "Pathological geometry detected. Bypassing MAP and using standard jitter..."
        )
        ...
        opt_result = scipy.optimize.minimize(
            scipy_objective, np.array(initial_positions[0]), method="BFGS", jac=True
        )
        map_estimate = jnp.array(opt_result.x)
        warm_start_positions = map_estimate + jnp.where(
            map_estimate == 0, jitter, map_estimate * 0.01
        )
    # -----------------------------------------------
    ...
    def warmup_step(carry, step_key):
        ...
        def single_chain_warmup(key, state, da_state):
            ...
            next_da_state = dual_averaging_step(
                da_state, acceptance_rate, next_state.position, target_rate=target_rate
            )
            ...
            next_da_state = next_da_state._replace(
                ...
                log_step_size_bar=jnp.maximum(
                    next_da_state.log_step_size_bar, min_log_step
                ),
            )

            return next_state, next_da_state

        next_states, next_da_states = jax.vmap(single_chain_warmup)(
            keys, states, da_states
        )
    ...
    def sample_step(carry_states, step_key):
        ...
        next_states, positions = jax.vmap(single_chain_sample)(
            keys, carry_states, final_step_sizes, final_choleskys
        )
...
if __name__ == "__main__":
    ...
    def make_horseshoe():
        ...
        def logdensity(theta_flat):
            ...
            log_prior_beta = jnp.sum(
                -0.5 * jnp.log(2 * jnp.pi * var_beta) - 0.5 * (beta**2 / var_beta)
            )

            mu = X @ beta
            log_lik = jnp.sum(
                -0.5 * jnp.log(2 * jnp.pi * true_sigma**2)
                - 0.5 * ((y - mu) / true_sigma) ** 2
            )
    ...
    def make_lgcp():
        ...
        coords = jnp.stack(jnp.meshgrid(jnp.arange(M), jnp.arange(M)), axis=-1).reshape(
            -1, 2
        )
        dists_sq = jnp.sum((coords[:, None, :] - coords[None, :, :]) ** 2, axis=-1)
        Sigma = sigma_val**2 * jnp.exp(
            -dists_sq / (2.0 * l_val**2)
        ) + 1e-6 * jnp.eye(D)
