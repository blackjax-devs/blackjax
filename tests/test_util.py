...
import numpy as np
from jax import numpy as jnp, random as jr, jit, tree, vmap
from absl.testing import absltest, parameterized

import blackjax
from blackjax.util import run_inference_algorithm, store_only_expectation_values, thin_kernel, thin_algorithm
...
class ThinInferenceAlgorithmTest(chex.TestCase):
    ...
    logdf = lambda self, x: - ((x[::2] - 1)**2 + (x[1::2] - x[::2]**2)**2).sum(-1) / 2 # Rosenbrock
    d = 2
    init_pos = jnp.ones(d)
    ...
    def warmup(self, rng_key, num_steps, thinning:int=1):
        ...
        n_steps *= thinning
        config = config._replace(L=config.L * thinning) # NOTE: compensate L for thinning
        return state, config, n_steps

    def run(self, rng_key, state, config, num_steps, thinning:int=1):
        sampler = blackjax.mclmc(self.logdf, L=config.L, step_size=config.step_size, 
                            inverse_mass_matrix=config.inverse_mass_matrix,)
        if thinning!=1:
            sampler = thin_algorithm(sampler, thinning=thinning, info_transform=lambda info: tree.map(jnp.mean, info))
    ...
    def test_thin(self):
        ...
        state, config, n_steps = jit(vmap(partial(self.warmup, num_steps=self.num_steps, thinning=1)))(self.rng_keys)
        config = tree.map(lambda x: jnp.median(x, 0), config)
        state_thin, config_thin, n_steps_thin = jit(vmap(partial(self.warmup, num_steps=self.num_steps, thinning=4)))(self.rng_keys)
        ...
        np.testing.assert_allclose(config_thin.inverse_mass_matrix, config.inverse_mass_matrix, rtol=rtol)


        # Test thin algorithm in run
        state, history = jit(vmap(partial(self.run, config=config, num_steps=self.num_steps, thinning=1)))(self.rng_keys, state)
        samples = jnp.concatenate(history[0].position)
        state_thin, history_thin = jit(vmap(partial(self.run, config=config_thin, num_steps=self.num_steps, thinning=4)))(self.rng_keys, state_thin)
        samples_thin = jnp.concatenate(history_thin[0].position)

        rtol, atol = 1e-1, 1e-1
        np.testing.assert_allclose(samples_thin.mean(0), samples.mean(0), rtol=rtol, atol=atol)
        np.testing.assert_allclose(jnp.cov(samples_thin.T), jnp.cov(samples.T), rtol=rtol, atol=atol)
